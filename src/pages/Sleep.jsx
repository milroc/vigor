...
// Sleep-stage palette, shared across the scrubber, skyline, and legend. Each color
// is a CSS variable (defaults in global.css :root) so the in-tab COLOR TUNER can
// re-theme every chart at runtime via document.documentElement.style.setProperty.
const STAGE = { deep: 'var(--st-deep)', core: 'var(--st-core)', rem: 'var(--st-rem)', awake: 'var(--st-awake)' };
const INBED_PRE = 'var(--inbed-pre)';   // time in bed before sleep onset (lighter gray)
const INBED_POST = 'var(--inbed-post)'; // time in bed after final wake (darker gray)
const RESP = 'var(--c-resp)';
const SPO2 = 'var(--c-spo2)';
const DIST = 'var(--c-dist)';    // breathing disturbances
const FULLWAKE = 'var(--c-fullwake)'; // minutes in full (>=10m) awakenings
const HR = 'var(--c-hr)';
const HRV = 'var(--c-hrv)';
const NAP = 'var(--c-nap)';
const DEBT = 'var(--c-debt)';
const LOAD = 'var(--c-load)';    // training load (active energy)
const RHR = 'var(--c-rhr)';      // following-day resting HR (recovery output)
const DAYLIGHT = 'var(--c-daylight)'; // time in daylight (bedtime input)
// Consistency has its OWN palette (must not reuse stage colors) — its own vars.
const BED_TGT = 'var(--tgt-bed)';
const WAKE_TGT = 'var(--tgt-wake)';
...
// The default (ORIGINAL, pre-audit) palette — also the "revert" target per role.
...
const TUNER_JS_NAME = {
  '--st-deep': 'STAGE.deep', '--st-core': 'STAGE.core', '--st-rem': 'STAGE.rem', '--st-awake': 'STAGE.awake',
  '--tgt-bed': 'BED_TGT', '--tgt-wake': 'WAKE_TGT',
  '--inbed-pre': 'INBED_PRE', '--inbed-post': 'INBED_POST',
  '--c-hr': 'HR', '--c-hrv': 'HRV', '--c-resp': 'RESP', '--c-spo2': 'SPO2', '--c-dist': 'DIST',
  '--c-nap': 'NAP', '--c-debt': 'DEBT', '--c-load': 'LOAD',
};
...
function resolveVar(over, k) { return (over[k] || TUNER_DEFAULTS[k]).toLowerCase(); }

const TUNER_LS = 'sleepTunerV1';
...
function TunerPanel({ onClose }) {
  ...
  const exportText = useMemo(() => {
    ...
    const js = [
      '// Sleep.jsx color constants (resolved hexes from the tuner):',
